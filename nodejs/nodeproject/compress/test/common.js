...
seajs.use("ddWeixin", function(ddWeixin){
	ddWeixin.autoOpenId("wxf91bab01569cc168", {appType:1, isDebug: true}).done(function(openId){
		console.log("授权成功");
		// 全功能的真正入口
		// 但是不从这里开始，也木有什么大问题......
		// 主要是用户信息，可能有延迟
...
define("nav", function(require, exports, module){
	...
	add("make", {
		...
		click : function (e) {
			var href = "/tougao/templateList.html";
			// 在APP内 & 是首页
			if (window.isIndex && window.DDInnerJS) {
				DDInnerJS.openNewActivity(G.origin + href);
			} else {
	...
	add("user", {
		...
		click : function (e) {
			...
			USER.afterCheckLogin(function(){
				var link = "/uc/usercenter.html";
				// 在APP内 & 是首页
				if (window.isIndex && window.DDInnerJS) {
					DDInnerJS.openNewActivity(G.origin + link);
					return;
				}
